...
// Weaker/touch devices get fewer particles, a capped pixel ratio, no MSAA
// and a smaller bloom buffer so the animation stays smooth.
const isMobile = isMobileDevice();
const particleCount = isMobile ? 5000 : 10000;

// Reused every frame instead of allocating a new THREE.Color per particle -
// that allocation churn was the main cause of stutter on phones.
...
const themes = {
  ...
  pearl: {
    ...
    colors: [
      ...
      new THREE.Color(0xb0b0b8),
      new THREE.Color(0xfafaff)
    ],
...
function isMobileDevice() {
  ...
  const hasSmallScreen = window.innerWidth <= 820;
  return hasCoarsePointer || hasSmallScreen;
}

function getBloomResolution() {
  const scale = isMobile ? 0.5 : 1;
  return new THREE.Vector2(window.innerWidth * scale, window.innerHeight * scale);
}
...
function init() {
  ...
  const bloomPass = new UnrealBloomPass(getBloomResolution(), 1.5, 0.4, 0.85);
...
function createParticleSystem() {
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    const size = getAttributesForParticle(i);
    colors[i3] = _scratchColor.r;
    colors[i3 + 1] = _scratchColor.g;
    colors[i3 + 2] = _scratchColor.b;
    sizes[i] = size;
...
// Mutates the shared _scratchColor and returns just the size, so this can
// run 5,000-10,000 times per frame without allocating new objects each time.
function getAttributesForParticle(i) {
  ...
  _scratchColor.copy(color1).lerp(color2, blendFactor).multiplyScalar(0.65 + Math.random() * 0.55);
...
function animateParticles() {
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    const baseParticleSize = getAttributesForParticle(i);
    ...
    particleColors[i3] = _scratchColor.r * brightnessFactor;
    particleColors[i3 + 1] = _scratchColor.g * brightnessFactor;
    particleColors[i3 + 2] = _scratchColor.b * brightnessFactor;
...
function onWindowResize() {
  ...
  if (bloomPass) {
    const res = getBloomResolution();
    bloomPass.setSize(res.x, res.y);
  }
}
...
function updateParticleColorsAndSizes() {
  ...
  for (let i = 0; i < particleCount; i++) {
    const size = getAttributesForParticle(i);
    pColors[i * 3] = _scratchColor.r;
    pColors[i * 3 + 1] = _scratchColor.g;
    pColors[i * 3 + 2] = _scratchColor.b;
    pSizes[i] = size;
  }
